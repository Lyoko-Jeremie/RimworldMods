...
**必须使用 `rimsage` MCP 工具查询游戏原始代码和游戏的 `Assembly-CSharp.dll` 的反编译源码，以便更好地理解游戏逻辑，并确认对应函数是否存在。**
...
在 `../../..` 目录下执行 `git log --name-only -n <数字>` 查询最近的文件变更提交信息

## 此项目的专有词汇
- 超维科技 (OuterrealmTech)
- 超维存储 (OuterrealmStorage)
- 牵引光束搬运器 (Manipulator Beam Emitter)
